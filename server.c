...
#include <stdarg.h>
#include <ctype.h>
// define the maximum size for both
// input buffer and password buffer
#define BUFFMAX 32

// global variables nicely grouped
struct {
    int port; // listening port
    char buffer[BUFFMAX]; // temporary buffer for input
    char password[BUFFMAX]; // required password
...
void readLineFromFd( int fd, char * buff) {
    ...
    while(1) {
        ...
        if( * (ptr - 1) == '\n') break;
        // if counter is greater or equal to buffer limit
        // buffer overflow exploit detected and stopped
        if(counter >= (BUFFMAX-1)) {
